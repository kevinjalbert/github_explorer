...
individual log files. It is still possible to manually orchestrate the multiple
GitHub Explorer programs by making use of the -p and -m parameters in the
GitHub Explorer program.

"""

def task(language, keywords, sourceStatements, clone, processNumber, maxProcesses):
  ...
  gitHubExplorer = github_explorer.GitHubExplorer(language, keywords, sourceStatements, clone, processNumber, maxProcesses, True, logger)
...
if __name__ == '__main__':
  ...
  parser = argparse.ArgumentParser(
      description = 'github_explorer_driver <https://github.com/kevinjalbert/github_explorer>',
  ...
  parser.add_argument(
      ...
      default=1,
      dest='maxProcesses',
      help='The maximum number of concurrent executing processes to be used in the search)')
  parser.add_argument(
      '-c',
      action='store_true',
      default=False,
  ...
  for processNumber in range(int(userArgs.maxProcesses)):
    worker = threading.Thread(target=task, args=(userArgs.language, userArgs.keywords, userArgs.sourceStatements, userArgs.clone, processNumber + 1, int(userArgs.maxProcesses)))
